...
def status_bar(d, k, label):
    ...
    text(d, W - 28, H - 24, "oktanio.dev", F_MONO_XS, fade(FAINT, k), anchor="ra")
...
TERM_LINES = [
    ...
    (0,  0, [("yohanesokta", MUTED), (" on ", FAINT), ("master", MUTED),
             (" [!?] ", FAINT), ("took 16s", FAINT)], False),
    (2,  0, [("→ ", ACCENT), ("ssh yohanesokta@localhost", BODY)], True),
...
JOKE_LINES = [
    (0,  0, [("yohanesokta", MUTED), (" on ", FAINT), ("master", MUTED),
             (" [!?]", FAINT)], False),
    (2,  0, [("→ ", ACCENT), ("netstat -tulpn", BODY)], True),
    (15, 1, [("active internet connections", BODY)], False),
    (18, 1, [("routing all traffic.", MUTED)], False),
]
...
def scene_identity(d, f):
    ...
    if t >= 1:
        text(d, x0, 78, "Yohanes Oktanio", F_DISP_XL, HEADING)
...
def scene_trajectory(d, f):
    ...
    n = int(max(0, t) * 2.4)
    text(d, 54 + 2 * CW, 54, "https://yohanes.oktanio.dev"[:n], F_MONO, BODY)
    ...
    if t >= 20:
        a = ease_out(seg(t, 20, 6))
        ctext(d, W / 2, H - 66,
              "roadmap: linux kernel development · advanced networking · custom routing protocols",
...
def scene_sign(d, f):
    ...
    if t >= 3:
        ...
        ctext(d, cx, 206, "yohanesokta", F_MONO_XL, fade(HEADING, b))
    ...
    if t >= 11 and (f // 4) % 2 == 0:
        half = text_w("yohanesokta", F_MONO_XL) / 2.0
        cwx = cw(F_MONO_XL)
        x = cx + half + cwx * 0.35
